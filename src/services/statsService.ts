...
export const getMessageDistribution = async (companyId: string): Promise<MessageDistribution> => {
  const company = await Company.findById(companyId);
  if (!company) {
...
export const getGrowthMetrics = async (companyId: string): Promise<GrowthMetrics> => {
  const company = await Company.findById(companyId);
  if (!company) {
    return {
      rating: 0,
