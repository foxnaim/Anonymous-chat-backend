 ...
 */
export const cacheHeaders = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (req.method === 'GET' && !req.query.page && !req.query.limit) {
    ...
    if (req.path.includes('/companies/code/') || req.path.includes('/companies/public') || req.path.includes('/plans')) {
      res.set({
